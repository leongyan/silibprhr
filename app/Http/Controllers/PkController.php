...
class PkController extends Controller
{
    ...
    {   
        ...
        $pks = DB::table($db.'_pk')
                ->join($db.'_karyawan', $db.'_karyawan.id', '=', $db.'_pk.id_kry')
                ->select($db.'_karyawan.nama', $db.'_pk.*')
                ->orderBy('tgl_mulai')
                ->get();

        foreach ($pks as $p){
            $p->id_pk = Carbon::create($p->tgl_mulai)->format('ym').sprintf('%03d', $p->id);
            if(Carbon::now()->lt(Carbon::create($p->tgl_akhir))){
                $p->sts = 'AKTIF';
            }
            else{
                $p->sts = 'NONAKTIF';
